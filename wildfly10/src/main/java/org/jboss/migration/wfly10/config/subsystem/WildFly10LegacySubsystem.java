...
public class WildFly10LegacySubsystem extends WildFly10Subsystem {
     ...
     * @param subsystemManagement the subsystem management
     * @param context the task context
     * @throws Exception if there was a failure processing the warnings
     */
    protected void processWarnings(List<String> migrationWarnings, WildFly10SubsystemManagement subsystemManagement, ServerMigrationTaskContext context) throws Exception {
    }
    ...
    public ServerMigrationTask getServerMigrationTask(final WildFly10SubsystemManagement subsystemManagement) {
        ...
        return new ServerMigrationTask() {
            ...
            public ServerMigrationTaskResult run(ServerMigrationTaskContext context) throws Exception {
                ...
                final ModelNode subsystemConfig = subsystemManagement.getSubsystem(subsystemName);
                if (subsystemConfig == null) {
                    return ServerMigrationTaskResult.SKIPPED;
                }
                context.getLogger().debugf("Migrating subsystem %s...", subsystemName);
                final PathAddress address = subsystemManagement.getSubsystemPathAddress(subsystemName);
                final ModelNode op = Util.createEmptyOperation("migrate", address);
                final ModelNode result = subsystemManagement.getConfigurationManagement().getModelControllerClient().execute(op);
                ...
                if(!SUCCESS.equals(outcome)) {
                    ...
                    processWarnings(migrateWarnings, subsystemManagement, context);
                    ...
                    if (subsystemManagement.getSubsystems().contains(subsystemName)) {
                        // remove itself after migration
                        subsystemManagement.removeSubsystem(subsystemName);
