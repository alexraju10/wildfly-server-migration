 ...
 */
package org.jboss.migration.wfly10.config.subsystem.ee;
...
public class AddDefaultBindingsConfig implements WildFly10SubsystemMigrationTaskFactory {
    ...
    @Override
    public ServerMigrationTask getServerMigrationTask(final ModelNode config, WildFly10Subsystem subsystem, WildFly10SubsystemManagement subsystemManagement) {
        return new WildFly10SubsystemMigrationTask(config, subsystem, subsystemManagement) {
            ...
            protected ServerMigrationTaskResult run(ModelNode config, WildFly10Subsystem subsystem, WildFly10SubsystemManagement subsystemManagement, ServerMigrationTaskContext context, TaskEnvironment taskEnvironment) throws Exception {
                ...
                final boolean addConcurrencyUtilitiesDefaultConfigSkipped = new TaskEnvironment(migrationEnvironment, org.jboss.migration.wfly10.config.subsystem.EnvironmentProperties.getSubsystemSubtaskPropertiesPrefix(subsystem.getName(), AddConcurrencyUtilitiesDefaultConfig.SERVER_MIGRATION_TASK_NAME.getName())).isSkippedByEnvironment();
                ...
                setupDefaultDatasource(defaultDataSourceJndiName, defaultDataSourceName, addOp, subsystemManagement, context, taskEnvironment, taskResultBuilder);
                setupDefaultJMSConnectionFactory(defaultJmsConnectionFactoryJndiName, defaultJmsConnectionFactoryName, addOp, subsystemManagement, context, taskEnvironment, taskResultBuilder);
            ...
            private void setupDefaultJMSConnectionFactory(String defaultJmsConnectionFactoryJndiName, String defaultJmsConnectionFactoryName, final ModelNode addOp, WildFly10SubsystemManagement subsystemManagement, final ServerMigrationTaskContext context, final TaskEnvironment taskEnvironment, final ServerMigrationTaskResult.Builder taskResultBuilder) throws Exception {
                // if the subsystem config defines expected default resource then use it
                final ModelNode subsystemConfig = subsystemManagement.getSubsystem(WildFly10SubsystemNames.MESSAGING_ACTIVEMQ);
            ...
            private void setupDefaultDatasource(String defaultDataSourceJndiName, final String defaultDataSourceName, final ModelNode addOp, WildFly10SubsystemManagement subsystemManagement, final ServerMigrationTaskContext context, final TaskEnvironment taskEnvironment, final ServerMigrationTaskResult.Builder taskResultBuilder) throws Exception {
                // if the subsystem config defines expected default resource then use it
                final ModelNode subsystemConfig = subsystemManagement.getSubsystem(WildFly10SubsystemNames.DATASOURCES);
                if (subsystemConfig == null) {
                    return;
                }
