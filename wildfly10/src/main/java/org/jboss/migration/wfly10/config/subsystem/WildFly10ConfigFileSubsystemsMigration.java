 ...
 * @author emmartins
 */
public class WildFly10ConfigFileSubsystemsMigration<S extends Server> {
    ...
    public WildFly10ConfigFileSubsystemsMigration(List<WildFly10Extension> supportedExtensions) {
    ...
    public ServerMigrationTask getManagementResourcesServerMigrationTask(final Path targetConfigFilePath, final WildFly10SubsystemManagement subsystemManagement) {
        return new ServerMigrationTask() {
            @Override
            public ServerMigrationTaskName getName() {
            ...
            public ServerMigrationTaskResult run(ServerMigrationTaskContext context) throws Exception {
                ...
                final List<WildFly10Extension> migrationExtensions = getMigrationExtensions(context.getServerMigrationContext().getMigrationEnvironment());
                migrateExtensions(subsystemManagement, migrationExtensions, context);
                return ServerMigrationTaskResult.SUCCESS;
            }
        };
    }
    ...
    protected void migrateExtensions(final WildFly10SubsystemManagement subsystemManagement, List<WildFly10Extension> extensionsToMigrate, ServerMigrationTaskContext context) throws IOException {
        for (WildFly10Extension extensionToMigrate : extensionsToMigrate) {
            extensionToMigrate.migrate(subsystemManagement, context);
        }
    }
